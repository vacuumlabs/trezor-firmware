...
class TestCardanoCbor(unittest.TestCase):
    ...
    def test_encode_chunked(self):
        for chunk_size in [1, 10, 42, 256, 2048]:
            large_dict = {i: i for i in range(100)}
            encoded = encode(large_dict)

            encoded_chunks = [
                bytes(chunk) for chunk in encode_chunked(large_dict, chunk_size)
            ]

            expected_number_of_chunks = math.ceil(len(encoded) / chunk_size)
            ...
            for i in range(len(encoded_chunks) - 1):
                self.assertEqual(len(encoded_chunks[i]), chunk_size)

            # last chunk should contain the remaining bytes or the whole chunk
            remaining_bytes = len(encoded) % chunk_size
            expected_last_chunk_size = remaining_bytes if remaining_bytes > 0 else chunk_size
