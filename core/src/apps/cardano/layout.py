...
async def _confirm_data_chunk(
    ...
    DISPLAYED_SIZE = 32  # TODO determine the best value
    displayed_bytes = first_chunk[:DISPLAYED_SIZE]
    optional_ellipsis = "" if data_size == DISPLAYED_SIZE else "..."
    await confirm_properties(
        ctx,
        br_type,
        title="Confirm transaction",
        props=[
            (
                f"{title} ({data_size} bytes):",
                f"{displayed_bytes}{optional_ellipsis}",
            )
        ],
        br_code=ButtonRequestType.Other,
    )
...
async def confirm_tx(
    ...
    if total_collateral is not None:
        props.append(("Total collateral:", format_coin_amount(fee, network_id)))
